...
export function computeScore(input: FinancialInput): ScoreResult {
  ...
  if (assetRatio >= 3) {
    ...
    reasons.push('Limited assets relative to income');
  }
  ...
  // --- Missed payments (max -25) ---
  if (input.missedPayments === 0) {
    score += 5;
  } else if (input.missedPayments <= 2) {
    score -= 10;
    reasons.push('Recent missed payments detected');
  } else {
    score -= 25;
    reasons.push('Multiple missed payments in last 2 years');
  }

  // Clamp to 0-100
  const aiScore = Math.max(0, Math.min(100, Math.round(score)));

  // Compute tiers for ZK circuit
  ...
  // Determine eligibility (mirrors the Compact circuit logic)
  let eligibility: ScoreResult['eligibility'];
  if (debtRatioTier === 2 && incomeTier === 0) {
    eligibility = 'INELIGIBLE';
  } else if (aiScore < 40) {
    eligibility = 'INELIGIBLE';
  } else if (aiScore > 70 && debtRatioTier < 2) {
  ...
  if (eligibility === 'ELIGIBLE' && reasons.length === 0) {
    reasons.push('Strong financial profile');
  }
...
// Generate a deterministic commitment hash from inputs (for on-chain submission)
export async function generateCommitment(input: FinancialInput): Promise<string> {
  const data = JSON.stringify(input) + Date.now().toString();
